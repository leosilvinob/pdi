...
void faces_load_metadata() {
    ...
    while (f) {
        ...
        if (name.endsWith("_info.txt")) {
            ...
            if (p > 0) {
                String person = name.substring(0, p);
                if (person.startsWith("/faces/")) person = person.substring(7);
                String vinc = f.readString();
                vinc.trim();
                vinculoMap[person.c_str()] = vinc.c_str();
            }
        }
        f.close();
        f = root.openNextFile();
...
void faces_load_db() {
    if (!sd_ready) return;
    if (!SD.exists(FACE_DB_PATH)) return;
    File file = SD.open(FACE_DB_PATH, FILE_READ);
    if (!file) return;
    int count = 0;
...
static void faces_save_db_entry(int id, const std::string &name) {
    ...
    entry.ctrl[0] = 0x14;
    entry.ctrl[1] = 0x08;
    File file = SD.open(FACE_DB_PATH, FILE_APPEND);
    if (!file) file = SD.open(FACE_DB_PATH, FILE_WRITE);
    if (file) {
        file.write((uint8_t *)&entry, sizeof(entry));
        file.close();
    } else {
        log_e("Failed to save face db entry");
    }
}

static void faces_save_metadata(const std::string &name, const std::string &vinc) {
    if (!sd_ready) return;
    String path = String("/faces/") + name.c_str() + "_info.txt";
...
static int enroll_from_jpeg(const std::string &name, const std::string &vinc, const uint8_t *jpg, size_t jpg_len)
{
    ...
    std::list<dl::detect::result_t> &candidates = s1.infer((uint8_t *)rgb.data(), {(int)240, (int)240, 3});
...
#if CONFIG_ESP_FACE_DETECT_ENABLED
static esp_err_t cadastrar_handler(httpd_req_t *req)
{
    ...
    DeserializationError err = deserializeJson(doc, body.data());
    if (err) {
        return httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "JSON parse failed");
    }
...
void startCameraServer()
{
    ...
    // Bump httpd stack to avoid canary on heavy handlers (/cadastrar + enroll)
    config.stack_size = 16384; // bump to avoid canary in heavy handlers
...
#if CONFIG_ESP_FACE_DETECT_ENABLED
    httpd_uri_t cadastrar_uri = {
        .uri = "/cadastrar",
        .method = HTTP_POST,
        .handler = cadastrar_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
...
#if CONFIG_ESP_FACE_DETECT_ENABLED
    // Enable detection/recognition by default for periodic loop
    detection_enabled = 1;
    recognition_enabled = 1;
#endif
}
...
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED
// Simple periodic recognizer (called from loop)
int faces_periodic_recognize()
{
    if (!detection_enabled) return -1;
    ...
    if (fb->format == PIXFORMAT_JPEG) {
        rgb888.resize(fb->width * fb->height * 3);
        if (!fmt2rgb888(fb->buf, fb->len, PIXFORMAT_JPEG, rgb888.data())) {
            esp_camera_fb_return(fb);
            return -1;
        }
        buf_ptr = rgb888.data();
        rfb.data = (uint8_t *)buf_ptr;
        rfb.bytes_per_pixel = 3;
        rfb.format = FB_BGR888;
    ...
    HumanFaceDetectMSR01 s1(0.1F, 0.5F, 10, 0.2F);
    HumanFaceDetectMNP01 s2(0.5F, 0.3F, 5);
    std::list<dl::detect::result_t> &candidates = s1.infer((uint8_t *)buf_ptr, shape);
    std::list<dl::detect::result_t> &results = s2.infer((uint8_t *)buf_ptr, shape, candidates);
#else
    HumanFaceDetectMSR01 s1(0.3F, 0.5F, 10, 0.2F);
    std::list<dl::detect::result_t> &results = s1.infer((uint8_t *)buf_ptr, shape);
#endif
